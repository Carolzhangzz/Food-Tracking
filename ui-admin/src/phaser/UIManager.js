...
export default class UIManager {
    ...
    createDayProgressBar() {
        ...
        this.progressBarBg.fillRoundedRect(progressX, progressY, progressBarWidth, progressBarHeight, 4);
        ...
        // 进度标签
        this.progressLabel = this.scene.add.text(width / 2, progressY + progressBarHeight + 15, '', {
            fontSize: '12px',
            fontFamily: 'monospace',
            fill: '#e2e8f0',
            align: 'center'
    ...
    showDayCompleteAnimation() {
        ...
        // 创建完成提示
        const completeText = this.scene.add.text(width / 2, height / 2, 
    ...
    showClueJournal() {
        ...
        // 标题
        const title = this.scene.add.text(width / 2, journalY + 40, 
        ...
        const progressText = this.scene.add.text(width / 2, journalY + 75,
            this.scene.playerData.language === 'zh' ? 
        ...
        if (this.clues.length === 0) {
            const noCluesText = this.scene.add.text(width / 2, journalY + journalHeight / 2, 
                this.scene.playerData.language === 'zh' ? '暂无线索\n开始与NPC对话来收集线索！' : 'No clues yet\nStart talking to NPCs to collect clues!', {
            ...
            this.clues.forEach((clue, index) => {
                // 天数标签
                const dayLabel = this.scene.add.text(journalX + 20, yOffset, 
                ...
                // NPC名称
                const npcName = this.scene.add.text(journalX + 20, yOffset, 
                    `${clue.npcName}:`, {
                    fontSize: Math.min(width * 0.025, 16) + 'px',
                    fontFamily: 'monospace',
                    fill: '#ffd700',
                ...
                // 线索内容
                const clueText = this.scene.add.text(journalX + 40, yOffset, 
                    clue.clue, {
    ...
    showFinalEgg(content) {
        ...
        // 标题
        const title = this.scene.add.text(width / 2, eggY + 50, 
            this.scene.playerData.language === 'zh' ? '🎉 恭喜完成7天旅程！' : '🎉 Congratulations on Completing the 7-Day Journey!', {
            fontSize: Math.min(width * 0.04, 28) + 'px',
            fontFamily: 'monospace',
            fill: '#ffd700',
            fontStyle: 'bold',
        ...
        // 副标题
        const subtitle = this.scene.add.text(width / 2, eggY + 90, 
            this.scene.playerData.language === 'zh' ? '师父为你准备的特别礼物：' : 'A special gift from your master:', {
        ...
        // 关闭按钮
        const closeBtn = this.scene.add.text(width / 2, eggY + eggHeight - 60, 
    ...
    handleMobileMovement(direction) {
        if (this.scene.agent && (!this.scene.dialogSystem || !this.scene.dialogSystem.isDialogActive())) {
