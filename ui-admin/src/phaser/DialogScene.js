...
import bac from "../assets/图像 2.png";
...
export default class DialogScene extends Phaser.Scene {
  ...
  create() {
    ...
    // 设置深色背景
    // 添加背景图片
    this.add.image(width / 2, height / 2, "npc").setDisplaySize(width, height);
    ...
    // 添加关闭按钮
    const closeButton = this.add.text(width - 50, boxY - 20, "✕", {
      fontSize: '24px',
      ...
      .on('pointerdown', () => {
        this.cameras.main.fadeOut(500, 15, 15, 35);
        this.cameras.main.once('camerafadeoutcomplete', () => {
          this.scene.start("MainScene");
        });
      })
      .on('pointerover', () => {
        closeButton.setFill('#ff4757');
      })
      .on('pointerout', () => {
        closeButton.setFill('#ff6b6b');
      });
