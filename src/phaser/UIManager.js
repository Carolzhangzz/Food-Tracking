...
export default class UIManager {
  ...
  handleResize() {
    // 🔧 简化 resize 处理，避免触发无限循环
    const width = window.innerWidth;
    const height = window.innerHeight;

    console.log(`📐 UIManager resize: ${width} x ${height}`);
  ...
  createSimpleMealProgress() {
    ...
    mealTypes.forEach((meal, i) => {
      ...
      const tip = () => {
        ...
        this.showNotification(map[meal.type] || "Progress", 2000);
      };
  ...
  createMoveHint() {
    ...
    const language = this.scene.playerData?.language || "zh";
    const hintText =
      language === "zh"
        ? "点击移动 | 点击NPC对话"
  ...
  addClue(clue) {
    ...
    const lang = this.scene.playerData?.language || "zh";
    this.showNotification(lang === "zh" ? "新线索!" : "New clue!");
  }
  ...
  showNotification(message, duration = 2500) {
    ...
    // 调整通知位置,避免与顶部对话框冲突
    const startY = this.isKeyboardOpen ? 80 : Math.min(200, height * 0.25);
  ...
  updateNotificationPositions() {
    const startY = this.isKeyboardOpen ? 80 : 200;
    this.notifications.forEach((n, i) => {
      if (n && n.setPosition) {
        n.y = startY + i * 35;
      }
    ...
    if (this.clues.length === 0) {
      ...
      this.clues.slice(0, 6).forEach((clue, idx) => {
        ...
        const dayLabel = this.scene.add.text(
          x + 15,
          offY,
          lang === "zh" ? `第${clue.day}天` : `Day ${clue.day}`,
          {
            fontSize: "11px",
            fontFamily: "monospace",
            fill: "#fbbf24",
        ...
        const shortClue = this.getShortClue(clue.clue);
        ...
        clueText.on("pointerdown", () => this.showFullClue(clue));
  ...
  showFinalEgg(egg) {
    ...
    this.showNotification(
      lang === "zh" ? "彩蛋已生成(UI 略)" : "Final egg (UI omitted)"
    );
  }
