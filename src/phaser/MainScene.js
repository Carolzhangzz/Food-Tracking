...
export default class MainScene extends Phaser.Scene {
    ...
    setupMap() {
        ...
        this.mapScale = Math.min(scaleX, scaleY);  // 统一缩放比例

        mainLayer.setScale(this.mapScale);  // 使用统一缩放
        mainLayer.setPosition(0, 0);

        this.mapScaleX = this.mapScale;  // 同步x/y缩放为统一值
        this.mapScaleY = this.mapScale;
    }
    ...
    setupGameSystems() {
        ...
        // 关键修改：在此处创建NPCManager（原代码在create()末尾）
        this.npcManager = new NPCManager(
            this,
            this.mapScale,  // 传递地图缩放比例
            this.playerContext  // 传递玩家上下文
        );
    ...
    onDialogComplete(npcId) {
        // 从NPC获取线索并添加到UI
        const clue = this.npcManager.getClueForNPC(npcId);
        if (clue) {
            this.onClueReceived(clue);
        }
        // 标记NPC为已对话
        this.npcManager.markNPCAsTalked(npcId);
        // 检查当日进度是否完成
    ...
    handleResize(gameSize) {
        ...
        // 关键修改：不仅更新缩放，还要重新设置NPC的高亮和点击区域
        this.npcManager?.updateScale(scale * 0.5);
        Array.from(this.npcManager?.npcs?.values() || []).forEach(npc => {
            if (npc.isUnlocked) {
                // 重新设置高亮效果位置
                if (npc.glowEffect) {
                ...
                // 重新创建点击区域（避免缩放后点击偏移）
                if (npc.day === this.npcManager.getCurrentDay()) {
    ...
    getGameState() {
        return {
            ...
            playerPosition: this.gridEngine?.getPosition("player") || {x: 0, y: 0},
            unlockedNPCs: Array.from(this.npcManager?.npcs?.values() || []).filter(npc => npc.isUnlocked).map(npc => npc.id),
            completedNPCs: Array.from(this.npcManager?.npcs?.values() || []).filter(npc => npc.hasCompletedDialog).map(npc => npc.id)
        };
    }
    ...
    shutdown() {
        ...
        // 关键修改：清理NPC相关资源
        if (this.npcManager) {
            this.npcManager.destroy();  // 调用NPCManager的destroy方法
        }
        ...
        // 清理UI元素
        if (this.uiManager) {
            this.uiManager.destroy();  // 假设UIManager有destroy方法
        }
    }
}